  ...
  return (
    <main className="pt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((s, i) => (
              <ScrollReveal key={s.title} delay={i * 0.06}>
                <div className="bg-card border border-[hsl(var(--wd-warm-grey))] rounded-[12px] p-6 hover:shadow-md hover:border-[hsl(var(--wd-blue))]/30 transition-all duration-200 h-full flex flex-col shadow-sm">
                  <s.icon className="w-8 h-8 text-primary mb-4" />
                  <h3 className="text-h3 text-foreground mb-1">{s.title}</h3>
                  {s.subtitle && <p className="text-caption text-primary mb-2">{s.subtitle}</p>}
                  <p className="text-body-sm text-muted-foreground mt-1 flex-1">{s.desc}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
      ...
      <section className="bg-muted py-20 md:py-28">
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <ScrollReveal>
                <span className="text-overline text-primary mb-4 block">Deep dive</span>
                <h2 className="text-h1 text-foreground mb-6">Technical SEO & audits</h2>
              </ScrollReveal>
              <ScrollReveal delay={0.1}>
                <div className="space-y-4">
                  {seoDetails.map((d) => (
                    <div key={d} className="flex gap-3 items-start">
                      ...
                      <p className="text-body text-muted-foreground">{d}</p>
                    </div>
                  ))}
                </div>
              </ScrollReveal>
            ...
            <div>
              <ScrollReveal delay={0.15}>
                <span className="text-overline text-primary mb-4 block">Deep dive</span>
                <h2 className="text-h1 text-foreground mb-6">LLM brand exposure</h2>
              </ScrollReveal>
              <ScrollReveal delay={0.2}>
                ...
                <div className="bg-card border border-[hsl(var(--wd-warm-grey))] rounded-[12px] p-6 shadow-sm">
                  <h4 className="text-h3 text-primary mb-3">Why it matters</h4>
      ...
      <section className="bg-wd-navy wd-ambient-glow py-20 md:py-28 text-center">
        <div className="container">
          <ScrollReveal>
            <h2 className="text-h1 mb-6 text-foreground">Ready to outperform?</h2>
          </ScrollReveal>
          <ScrollReveal delay={0.1}>
            <Link
              to="/contact"
